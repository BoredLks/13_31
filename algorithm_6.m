...
cache_decision = cal_cache_decision_0(h, total_users, F, iu_count_per_community, nf, v_chi_star, D_eg, D_iu, iu_indices, cache_preference); % 缓存优化算法
...
[r_decision,r_previous,~,~,~,buffer_state] = cal_Initialize_optimization_variables(chi, iu_count_per_community, nf, D_bf, iu_flags, cache_decision, community_users, requested_videos); % 初始化kkt参数
final_alltime_qoe = cal_final_alltime_qoe_without_adaption(nf, chi, Delta_t, alpha_qoe, beta_qoe, gamma_qoe, delta_qoe, D_bf, community_users, download_rates, initial_wait_times, r_decision, r_previous, buffer_state); % 计算最终QoE
